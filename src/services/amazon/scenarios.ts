...
export class ScenarioService extends BaseService {
    ...
    topNumericAssumptions.forEach(assumption => {
      scenarios.base.forEach(baseRow => {
        ...
        if (bearRow && bullRow && 
          ...
          const outcomeChange = this.formatOutcomeChange(baseRow.metric, avgChange, baseValue, bearValue, bullValue);
   ...
   */
  private async generateScenarios(context: ScenarioContext): Promise<{ bear: ScenarioRow[]; base: ScenarioRow[]; bull: ScenarioRow[] }> {
   ...
   */
  private applyScenarioChange(baseValue: number, metric: string, scenario: 'bear' | 'bull', scenarioPct: number): number {
   ...
   */
  private extractFinancialMetrics(basicCalc: FinancialModel): Array<{name: string; value: number; unit: string}> {
    const metrics = [
    ...
    Object.entries(basicCalc).forEach(([key, value]) => {
      if (!['revenue', 'costs', 'roi', 'npv'].includes(key.toLowerCase()) && typeof value === 'number') {
        metrics.push({
  ...
  private formatOutcomeChange(
    ...
    const isPercentageMetric = metric.toLowerCase().includes('roi') || 
                              metric.toLowerCase().includes('rate') ||
                              metric.toLowerCase().includes('margin');

    if (isPercentageMetric) {
      // For percentage metrics, show percentage points (pp)
      const avgAbsoluteChange = (Math.abs(bearValue - baseValue) + Math.abs(bullValue - baseValue)) / 2;
  ...
  private generateKeyDriverDescription(
    ...
    const impactLevel = topElasticity.sensitivity > 50 ? 'high' : 
                       topElasticity.sensitivity > 25 ? 'moderate' : 'low';
  ...
  private getCategoryContext(category: Assumption['category']): string {
    switch (category) {
      case 'financial': return ' - critical for financial projections';
      case 'market': return ' - drives market opportunity sizing';
      case 'competitive': return ' - affects competitive positioning';
      case 'technical': return ' - impacts delivery timeline and costs';
      default: return '';
    }
  }
  ...
  private isPositiveMetric(metric: string): boolean {
    const positiveMetrics = [
      'revenue', 'profit', 'roi', 'npv', 'margin', 'growth', 
      'users', 'conversion', 'retention', 'satisfaction'
    ];
    return positiveMetrics.some(positive => 
      metric.toLowerCase().includes(positive)
    );
  }
  ...
  private isCostMetric(metric: string): boolean {
    const costMetrics = [
      'cost', 'expense', 'spend', 'investment', 'budget', 
      'churn', 'risk', 'time', 'effort'
    ];
    return costMetrics.some(cost => 
  ...
  private inferUnit(metricName: string): string {
    ...
    if (name.includes('revenue') || name.includes('cost') || name.includes('npv') || name.includes('value')) {
      return 'USD';
    } else if (name.includes('rate') || name.includes('roi') || name.includes('margin') || name.includes('percent')) {
      return '%';
