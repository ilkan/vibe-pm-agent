...
export class QuickValidator {
  ...
  private performQuickValidation(
    idea: string,
    context?: QuickValidationContext
    ...
    const hasQuotaRisks = quotaRisks.some(risk => idea.toLowerCase().includes(risk.toLowerCase()));
  ...
  private generateThreeOptions(
    idea: string,
    verdict: 'PASS' | 'FAIL',
    context?: QuickValidationContext
  ): [QuickValidationOption, QuickValidationOption, QuickValidationOption] {
    if (verdict === 'FAIL') {
  ...
  private generateFailureOptions(
    _idea: string,
    _context?: QuickValidationContext
  ): [QuickValidationOption, QuickValidationOption, QuickValidationOption] {
    return [
      {
        id: 'A',
  ...
  private generateSuccessOptions(
    idea: string,
    context?: QuickValidationContext
  ): [QuickValidationOption, QuickValidationOption, QuickValidationOption] {
    const hasComplexity =
      idea.toLowerCase().includes('complex') || idea.toLowerCase().includes('system');
  ...
  private generateErrorRecoveryOptions(
    _idea: string
  ): [QuickValidationOption, QuickValidationOption, QuickValidationOption] {
    return [
      {
