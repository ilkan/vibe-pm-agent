...
export class SpecGenerator implements ISpecGenerator {
  ...
  formatSpecWithConsultingInsights(spec: KiroSpec, summary: ConsultingSummary): EnhancedKiroSpec {
    ...
    const alternativeOptions: ThreeOptionAnalysis =
      this.generateAlternativeOptionsFromInsights(summary);
  ...
  generateOptimizationNotes(optimizedWorkflow: OptimizedWorkflow): string[] {
    ...
    optimizedWorkflow.optimizations.forEach(opt => {
      switch (opt.type) {
        case 'batching':
          notes.push(
            `Batched ${opt.stepsAffected.length} operations to reduce API calls by ${opt.estimatedSavings.percentage}%`
          );
          break;
        case 'caching':
          notes.push(
            `Added caching layer to reduce redundant processing by ${opt.estimatedSavings.percentage}%`
          );
          break;
        case 'decomposition':
          notes.push(
            `Split workflow into ${opt.stepsAffected.length} smaller specs for better reusability`
          );
          break;
        case 'vibe_to_spec':
          notes.push(
            `Converted ${opt.estimatedSavings.vibes} vibes to ${opt.estimatedSavings.specs} specs for efficiency`
          );
          break;
      }
    });
  ...
  private generateDesign(optimizedWorkflow: OptimizedWorkflow): SpecDesign {
    ...
    const stepsByType = optimizedWorkflow.steps.reduce(
      (acc, step) => {
  ...
  private generateTasks(optimizedWorkflow: OptimizedWorkflow): SpecTask[] {
    ...
    optimizedWorkflow.optimizations.forEach((opt, index) => {
      tasks.push({
        ...
        estimatedEffort:
          opt.estimatedSavings.percentage > 30
            ? 'large'
            : opt.estimatedSavings.percentage > 15
              ? 'medium'
              : 'small',
  ...
  private generateMetadata(
    ...
    return {
      ...
      estimatedQuotaUsage: {
        vibesConsumed: optimizedWorkflow.steps.reduce(
          (sum, step) => sum + (step.type === 'vibe' ? step.quotaCost : 0),
          0
        ),
        specsConsumed: optimizedWorkflow.steps.reduce(
          (sum, step) => sum + (step.type === 'spec' ? step.quotaCost : 0),
          0
        ),
  ...
  private generateROIFromConsultingSummary(
  ...
  private generateAlternativeOptionsFromInsights(summary: ConsultingSummary): ThreeOptionAnalysis {
    ...
    const valueDriverInsight = summary.techniquesApplied.find(
      t => t.techniqueName === 'ValueDriverTree'
    );
    ...
    return {
      conservative: {
        name: 'Conservative',
        description:
          meceInsight?.actionableRecommendation || 'Minimal changes with systematic categorization',
        quotaSavings: 15,
      ...
      balanced: {
        name: 'Balanced',
        description:
          valueDriverInsight?.actionableRecommendation ||
          'Value-driven optimization with moderate changes',
        quotaSavings: 30,
      ...
      bold: {
        name: 'Bold',
        description:
          zeroBasedInsight?.actionableRecommendation || 'Radical redesign from first principles',
        quotaSavings: 50,
  ...
  private selectBestOption(scenarios: OptimizationScenario[]): string {
    ...
    const bestScenario = scenarios.reduce((best, current) => {
      const bestScore =
        best.savingsPercentage *
        (best.riskLevel === 'low' ? 1.5 : best.riskLevel === 'medium' ? 1.0 : 0.5);
      const currentScore =
        current.savingsPercentage *
        (current.riskLevel === 'low' ? 1.5 : current.riskLevel === 'medium' ? 1.0 : 0.5);
