...
export class ConsultingSummaryGenerator {
   ...
   */
  formatTechniqueInsights(
    technique: ConsultingTechnique,
    analysis: ConsultingAnalysis
  ): TechniqueInsight {
    let keyInsight = '';
    ...
    switch (technique.name) {
      case 'MECE':
        if (analysis.meceAnalysis) {
          ...
          actionableRecommendation = `Focus optimization efforts on the ${
            analysis.meceAnalysis.categories.sort(
              (a, b) => b.optimizationPotential - a.optimizationPotential
            )[0]?.name
          } category for maximum impact`;
        }
        break;

      case 'ValueDriverTree':
        if (analysis.valueDriverAnalysis) {
          const topDriver = analysis.valueDriverAnalysis.primaryDrivers.sort(
  ...
  createExecutiveSummary(
    ...
    const primaryRecommendation =
      recommendations[0]?.mainRecommendation || 'No specific recommendations available';
  ...
  private generateRecommendationsFromAnalysis(analysis: ConsultingAnalysis): string[] {
    ...
    if (analysis.meceAnalysis) {
      const topCategory = analysis.meceAnalysis.categories.sort(
        (a, b) => b.optimizationPotential - a.optimizationPotential
      )[0];
      if (topCategory) {
        recommendations.push(
          `Optimize ${topCategory.name} category to achieve ${topCategory.optimizationPotential}% quota reduction`
        );
      }
    }
    ...
    if (analysis.valueDriverAnalysis) {
      ...
      if (topDriver) {
        recommendations.push(
          `Focus on ${topDriver.name} optimization for ${topDriver.savingsPotential}% savings`
        );
      }
    }
    ...
    if (analysis.zeroBasedSolution) {
      recommendations.push(
        `Consider radical redesign: ${analysis.zeroBasedSolution.radicalApproach}`
      );
    }

    // Add general optimization recommendation
    recommendations.push(
      `Implement comprehensive optimization strategy to achieve ${analysis.totalQuotaSavings}% quota savings`
    );
  ...
  private generateEvidenceFromAnalysis(analysis: ConsultingAnalysis): Evidence[] {
    ...
    analysis.techniquesUsed.forEach(technique => {
      evidence.push({
        ...
        confidence:
          technique.relevanceScore > 0.7
            ? 'high'
            : technique.relevanceScore > 0.4
              ? 'medium'
              : 'low',
      });
    });
   ...
   */
  private compileSupportingEvidence(
    analysis: ConsultingAnalysis,
  ...
  private selectBestOption(threeOptions: ThreeOptionAnalysis): string {
    ...
    const scored = options.map(({ name, option }) => ({
      ...
      score:
        option.quotaSavings *
        (option.riskLevel === 'low' ? 1.2 : option.riskLevel === 'medium' ? 1.0 : 0.8),
    }));
