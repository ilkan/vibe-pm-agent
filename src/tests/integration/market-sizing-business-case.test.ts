...
describe('Market Sizing and Business Case Integration', () => {
  ...
  describe('Complete Market Sizing Workflow', () => {
    test('should perform comprehensive TAM/SAM/SOM analysis with multiple methodologies', async () => {
      const featureIdea =
        'AI-powered customer service automation platform for e-commerce businesses';
      ...
      marketData.assumptions.forEach(assumption => {
        expect(assumption.category).toMatch(
          /^(market-growth|penetration-rate|pricing|competition|technology)$/
        );
    ...
    test('should handle different sizing methodologies appropriately', async () => {
      ...
      for (const test of methodologyTests) {
        ...
        const avgReliability =
          marketData.methodology.reduce((sum, m) => sum + m.reliability, 0) /
          marketData.methodology.length;
        expect(avgReliability).toBeGreaterThanOrEqual(
          test.expectedCharacteristics.reliabilityRange[0]
        );
        expect(avgReliability).toBeLessThanOrEqual(
          test.expectedCharacteristics.reliabilityRange[1]
        );
  ...
  describe('Stakeholder Communication Integration', () => {
    test('should generate executive one-pager with integrated market sizing data', async () => {
      ...
      const onePagerArgs: ManagementOnePagerArgs = {
        ...
        design:
          'AI-powered fraud detection with machine learning algorithms and real-time processing',
      ...
      expect(markdown?.includes('market opportunity') || markdown?.includes('market size')).toBe(
        true
      );
    ...
    test('should generate PR-FAQ with market positioning and opportunity context', async () => {
      ...
      expect(markdown?.includes('market size') || markdown?.includes('market opportunity')).toBe(
        true
      );
    ...
    test('should generate stakeholder communication with competitive and market context', async () => {
      ...
      const stakeholderResult = await server.handleGenerateManagementOnePager(
        onePagerArgs,
        mockContext
      );
      ...
      const marketData = marketResult.content[0].json.data as MarketSizingResult;
      expect(
        presentation?.includes('TAM') || presentation?.includes('Total Addressable Market')
      ).toBe(true);
      expect(
        presentation?.includes('SAM') || presentation?.includes('Serviceable Addressable Market')
      ).toBe(true);
      ...
      expect(presentation?.includes('competitive') || presentation?.includes('competition')).toBe(
        true
      );
      expect(presentation?.includes('differentiation') || presentation?.includes('advantage')).toBe(
        true
      );
    ...
    test('should handle edge cases and provide appropriate fallbacks', async () => {
      ...
      for (const edgeCase of edgeCases) {
        ...
        if (edgeCase.expectedCharacteristics.lowerConfidence) {
          ...
          const avgConfidence =
            marketData.confidenceIntervals.reduce((sum, ci) => sum + ci.confidenceLevel, 0) /
            marketData.confidenceIntervals.length;
