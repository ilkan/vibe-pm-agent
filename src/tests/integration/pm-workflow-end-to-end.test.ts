...
describe('PM Workflow End-to-End Tests', () => {
  ...
  describe('Complete PM Workflow Tests', () => {
    it('should execute complete workflow from raw intent to all PM documents', async () => {
      const rawIntent =
        'I want to build a system that optimizes developer workflows and reduces quota consumption through intelligent analysis and consulting techniques.';
      ...
      const prfaq = await pmGenerator.generatePRFAQ(requirementsDoc, designDoc, '2025-06-01');
    ...
    it('should maintain document consistency across workflow phases', async () => {
      const rawIntent =
        'Create an AI-powered quota optimization system for developers using consulting methodologies.';
      ...
      const taskNames = taskPlan.immediateWins
        .concat(taskPlan.shortTerm)
        .map(t => t.name)
        .join(' ');
      ...
      const designRisks = Object.values(designOptions.options)
        .flatMap(o => o.majorRisks)
        .join(' ');
    ...
    it('should handle realistic PM scenarios with stakeholder needs', async () => {
      const scenarios = [
        {
          ...
          intent:
            'Build an enterprise-grade system that integrates with existing developer tools and provides ROI tracking for management.',
          context: {
        ...
        },
        {
          ...
          intent:
            'Create a lightweight quota optimization tool that can be built quickly with limited resources.',
          context: {
        ...
        },
        {
          ...
          intent:
            'Develop a comprehensive developer productivity platform with advanced analytics and optimization.',
          context: {
      ...
      for (const scenario of scenarios) {
        const requirements = await pmGenerator.generateRequirements(
          scenario.intent,
          scenario.context
        );
        ...
        if (scenario.name.includes('Enterprise')) {
          expect(requirements.businessGoal.toLowerCase()).toMatch(
            /(enterprise|integration|scale|security)/
          );
          expect(
            requirements.constraintsRisks.some(r => r.toLowerCase().includes('enterprise'))
          ).toBe(true);
        }
        ...
        if (scenario.name.includes('Growth')) {
          expect(
            requirements.userNeeds.gains.some(g => g.toLowerCase().includes('analytics'))
          ).toBe(true);
          expect(
            requirements.functionalRequirements.some(r => r.toLowerCase().includes('advanced'))
          ).toBe(true);
        }
        ...
        const onePager = await pmGenerator.generateManagementOnePager(
          requirementsDoc,
          formatDesignOptionsAsDocument(designOptions)
        );
  ...
  describe('PM Document Performance Tests', () => {
    it('should generate PM documents within acceptable time limits', async () => {
      const rawIntent =
        'Build a quota optimization system with consulting analysis and ROI forecasting.';
