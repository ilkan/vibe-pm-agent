...
describe('AIAgentPipeline Integration Tests', () => {
  ...
  describe('processIntent', () => {
    it('should process simple intent and return enhanced Kiro spec', async () => {
      const intent =
        'Create a user authentication system that validates credentials and manages sessions';
    ...
    it('should process complex intent with multiple operations', async () => {
      const intent =
        'Build a data processing pipeline that fetches user data from multiple APIs, validates it, transforms it, and stores it in a database with caching';
    ...
    it('should process intent with optional parameters', async () => {
      ...
      const consultingSummary = result.enhancedKiroSpec?.consultingSummary;
      expect(
        consultingSummary?.keyFindings.some(
          finding => finding.includes('cost') || finding.includes('performance')
        )
      ).toBe(true);
    });
        ...
        techniquesUsed: [
          {
            name: 'MECE' as const,
            relevanceScore: 0.9,
            applicableScenarios: ['quota optimization'],
          },
          {
            name: 'ValueDriverTree' as const,
            relevanceScore: 0.8,
            applicableScenarios: ['cost analysis'],
          },
        ],
    ...
    it('should apply different consulting techniques based on intent complexity', async () => {
      ...
      const complexIntent =
        'Build a comprehensive user management system with authentication, authorization, profile management, and audit logging';
      ...
      const simpleTechniques =
        simpleResult.enhancedKiroSpec?.consultingSummary.techniquesApplied.length || 0;
      const complexTechniques =
        complexResult.enhancedKiroSpec?.consultingSummary.techniquesApplied.length || 0;
