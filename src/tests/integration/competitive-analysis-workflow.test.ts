...
describe('Competitive Analysis Workflow Integration', () => {
  ...
  describe('End-to-End Competitive Analysis Workflow', () => {
    test('should complete full competitive analysis from feature idea to strategic recommendations', async () => {
      const featureIdea =
        'AI-powered project management tool that automatically optimizes developer workflows and reduces technical debt through intelligent code analysis and task prioritization';
      ...
      const competitiveResult = await server.handleAnalyzeCompetitorLandscape(
        competitiveArgs,
        mockContext
      );
      ...
      expect(analysisData.competitiveMatrix.rankings.length).toBe(
        analysisData.competitiveMatrix.competitors.length
      );

      // Validate SWOT analysis
      expect(analysisData.swotAnalysis).toBeDefined();
      expect(analysisData.swotAnalysis.length).toBe(
        analysisData.competitiveMatrix.competitors.length
      );
      ...
      expect(analysisData.marketPositioning.competitorPositions.length).toBe(
        analysisData.competitiveMatrix.competitors.length
      );
    ...
    test('should handle different analysis depths appropriately', async () => {
      ...
      for (const depth of depths) {
        ...
        expect(analysisData.competitiveMatrix.competitors.length).toBeLessThanOrEqual(
          expectedCompetitorCounts[depth]
        );
    ...
    test('should validate competitive analysis provides comprehensive strategic insights', async () => {
      ...
      const competitiveAnalysisData = competitiveResult.content[0].json
        .data as CompetitorAnalysisResult;
      ...
      expect(
        competitiveAnalysisData.marketPositioning.recommendedPositioning.length
      ).toBeGreaterThan(0);
    ...
    test('should meet performance benchmarks for competitive analysis', async () => {
      const testCases = [
        ...
        },
        {
          ...
          featureIdea:
            'Enterprise resource planning system with AI-powered analytics, multi-tenant architecture, and advanced reporting capabilities',
  ...
  describe('Error Handling and Edge Cases', () => {
    test('should handle insufficient competitor data gracefully', async () => {
      ...
      expect(
        analysisData.strategicRecommendations.some(
          rec =>
    ...
    test('should validate input parameters and provide helpful error messages', async () => {
      ...
      for (const testCase of invalidArgs) {
        try {
          await server.handleAnalyzeCompetitorLandscape(
            testCase.args as CompetitiveAnalysisArgs,
            mockContext
          );
    ...
    test('should handle timeout scenarios appropriately', async () => {
      ...
      const args: CompetitiveAnalysisArgs = {
        feature_idea:
          'Complex multi-industry platform with extensive feature set requiring comprehensive analysis across multiple market segments and competitive landscapes',
        analysis_depth: 'comprehensive',
      };
  ...
  describe('Data Quality and Validation', () => {
    ...
    test('should validate competitive matrix consistency', async () => {
      ...
      const totalWeight = matrix.evaluationCriteria.reduce(
