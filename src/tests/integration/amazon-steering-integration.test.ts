...
import {
  ...
  BusinessInputs,
  SteeringPackage,
} from '../../services/amazon/steering-writer';
...
describe('Amazon Steering Integration', () => {
      ...
      keyDrivers: [
        {
          ...
          impact: 18,
          description:
            'Product Pricing shows moderate sensitivity (±18% impact on Revenue) - critical for financial projections',
        },
      ],
      sensitivityPct: 20,
    };
    ...
    mockHardQuestions = [
      {
        id: 1,
        question:
          'How do you know that Product Pricing ($299) is accurate? What evidence supports this critical assumption?',
        targetAssumptions: ['A1'],
        category: 'financial',
        severity: 'critical',
        evidenceNeeded: [
          'Credible sources for Product Pricing',
  ...
  describe('Steering Package Creation', () => {
    it('should create complete steering package with all components', () => {
      ...
      expect(package_.attachments.map(a => a.type)).toEqual([
        'assumptions',
        'citations',
        'scenarios',
      ]);
    ...
    it('should create decision one-pager package correctly', () => {
      ...
      expect(package_.frontMatter.title).toBe(
        'Decision One-Pager — AI-Powered Analytics Dashboard'
      );
