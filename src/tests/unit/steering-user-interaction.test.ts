...
describe('SteeringUserInteraction', () => {
  ...
  describe('generatePreview', () => {
    ...
    it('should generate validation warnings for invalid content', () => {
      ...
      expect(preview.warnings).toContain(
        'File match pattern is required when inclusion rule is "fileMatch"'
      );
      expect(preview.warnings).toContain(
        'No file references found - steering file may not be well-connected'
      );
    });
  });
  ...
  describe('generateSummary', () => {
    ...
    it('should generate appropriate usage recommendations', () => {
      ...
      expect(
        summary.usageRecommendations.some(rec => rec.includes('new steering files created'))
      ).toBe(true);
      expect(
        summary.usageRecommendations.some(rec =>
