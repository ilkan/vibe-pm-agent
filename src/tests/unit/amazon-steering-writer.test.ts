...
describe('SteeringWriter', () => {
  ...
  describe('createSteeringPackage', () => {
    ...
    it('should generate correct attachment filenames with short hash', () => {
      ...
      expect(package_.frontMatter.paths.assumptions_json).toBe(`./attachments/assumptions-${shortHash}.json`);
      expect(package_.frontMatter.paths.citations_json).toBe(`./attachments/citations-${shortHash}.json`);
      expect(package_.frontMatter.paths.scenarios_json).toBe(`./attachments/scenarios-${shortHash}.json`);
    });
