...
describe('AIAgentPipeline Error Handling and Logging', () => {
  ...
  describe('Logging Functionality', () => {
    it('should log pipeline execution stages', async () => {
      ...
      expect(stageMessages.some(msg => msg.includes('Stage 4: Generating comprehensive ROI'))).toBe(
        true
      );
      expect(stageMessages.some(msg => msg.includes('Stage 5: Creating consulting-style'))).toBe(
        true
      );
      expect(stageMessages.some(msg => msg.includes('Stage 6: Generating enhanced Kiro'))).toBe(
        true
      );
      expect(stageMessages.some(msg => msg.includes('execution completed successfully'))).toBe(
        true
      );
    });
    ...
    it('should log performance metrics', async () => {
      ...
      const completionLog = infoLogs.find(log =>
        log.message.includes('execution completed successfully')
      );
    ...
    it('should log component-specific metrics', async () => {
      ...
      const optimizationLog = infoLogs.find(log =>
        log.message.includes('Workflow optimization completed')
      );
  ...
  describe('Error Recovery', () => {
    it('should provide fallback when no operations are identified', async () => {
      ...
      const warningLogs = mockConsoleWarn.mock.calls
        .map(parseLogSafely)
        .filter(log => log !== null);
      ...
      const hasOperationsWarning = warningLogs.some(log =>
        log.message.includes('No operations identified')
      );
  ...
  describe('Performance Monitoring', () => {
    ...
    it('should log stage-specific timing information', async () => {
      ...
      const stageCompletionLogs = infoLogs.filter(
        log => log.message.includes('completed') && !log.message.includes('execution completed')
      );
  ...
  describe('Robustness Testing', () => {
    ...
    it('should maintain system stability after errors', async () => {
      ...
      const infoLogs = mockConsoleLog.mock.calls.map(call => JSON.parse(call[0]));
      expect(infoLogs.some(log => log.message.includes('execution completed successfully'))).toBe(
        true
      );
    });
  });
