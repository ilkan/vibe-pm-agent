...
describe('SecureDocumentHandler', () => {
  ...
  describe('Document Processing', () => {
    ...
    it('should encrypt content when risk level is high', async () => {
      ...
      if (
        result.securityReport.riskLevel === 'high' ||
        result.securityReport.riskLevel === 'critical'
      ) {
    ...
    it('should handle processing errors gracefully', async () => {
      ...
      await expect(handler.processDocument('test content', invalidContext)).rejects.toThrow(
        'Secure document processing failed'
      );
    });
  });
  ...
  describe('Security Analysis', () => {
    ...
    it('should detect credential threats', async () => {
      ...
      const credentialThreats = result.securityReport.detectedThreats.filter(
        t => t.type === 'credentials'
      );
    ...
    it('should detect sensitive business data', async () => {
      ...
      const businessThreats = result.securityReport.detectedThreats.filter(
        t => t.type === 'sensitive_data'
      );
  ...
  describe('Encryption and Decryption', () => {
    ...
    it('should handle decryption errors gracefully', () => {
      ...
      expect(() => handler.decryptContent(invalidEncryptedData)).toThrow(
  ...
  describe('Error Handling', () => {
    it('should handle malformed context gracefully', async () => {
      ...
      await expect(handler.processDocument('test', malformedContext)).rejects.toThrow();
    });
