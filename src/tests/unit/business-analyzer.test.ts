...
describe('BusinessAnalyzer', () => {
  ...
  describe('selectTechniques', () => {
    ...
    it('should return techniques sorted by relevance score', () => {
      ...
      for (let i = 1; i < techniques.length; i++) {
        expect(techniques[i - 1].relevanceScore).toBeGreaterThanOrEqual(
          techniques[i].relevanceScore
        );
      }
    });
    ...
    it('should boost Zero-Based Design relevance for tight cost constraints', () => {
      ...
      const zeroBasedWithoutConstraints = techniquesWithoutConstraints.find(
        t => t.name === 'ZeroBased'
      );

      if (zeroBasedWithConstraints && zeroBasedWithoutConstraints) {
        expect(zeroBasedWithConstraints.relevanceScore).toBeGreaterThan(
          zeroBasedWithoutConstraints.relevanceScore
        );
      }
    });
    ...
    it('should boost Impact vs Effort Matrix for high user volume', () => {
      ...
      const impactEffortWithoutVolume = techniquesWithoutVolume.find(
        t => t.name === 'ImpactEffort'
      );

      if (impactEffortWithVolume && impactEffortWithoutVolume) {
        expect(impactEffortWithVolume.relevanceScore).toBeGreaterThan(
          impactEffortWithoutVolume.relevanceScore
        );
      }
    });
  });
  ...
  describe('applyMECE', () => {
    ...
    it('should identify optimization potential based on step types', () => {
      ...
      expect(vibeCategory!.optimizationPotential).toBeGreaterThan(
        analysisCategory!.optimizationPotential
      );
    });
  });
  ...
  describe('applyValueDriverTree', () => {
    it('should identify primary and secondary value drivers', () => {
      ...
      expect(analysis.primaryDrivers[0].currentCost).toBeGreaterThan(
        analysis.secondaryDrivers[0]?.currentCost || 0
      );
  ...
  describe('applyImpactEffortMatrix', () => {
    it('should categorize optimizations into impact/effort matrix', () => {
      ...
      expect(
        matrix.highImpactLowEffort.length + matrix.highImpactHighEffort.length
      ).toBeGreaterThan(0);
      expect(matrix.lowImpactLowEffort.length + matrix.lowImpactHighEffort.length).toBeGreaterThan(
        0
      );

      // Caching should be low effort
      const cachingOption = [...matrix.highImpactLowEffort, ...matrix.lowImpactLowEffort].find(
        opt => opt.name === 'caching'
      );
      expect(cachingOption).toBeDefined();

      // Decomposition should be high effort
      const decompositionOption = [
        ...matrix.highImpactHighEffort,
        ...matrix.lowImpactHighEffort,
      ].find(opt => opt.name === 'decomposition');
      expect(decompositionOption).toBeDefined();

      // Batching should be low impact (25% < 30% threshold)
      const batchingOption = [...matrix.lowImpactLowEffort, ...matrix.lowImpactHighEffort].find(
        opt => opt.name === 'batching'
      );
  ...
  describe('generateOptionFraming', () => {
    ...
    it('should adjust option savings for tight cost constraints', () => {
      ...
      const optionsWithConstraints = analyzer.generateOptionFraming(
        workflow,
        tightConstraintsParams
      );
      const optionsWithoutConstraints = analyzer.generateOptionFraming(workflow);

      expect(optionsWithConstraints.conservative.quotaSavings).toBeGreaterThan(
        optionsWithoutConstraints.conservative.quotaSavings
      );
