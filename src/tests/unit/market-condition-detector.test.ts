...
import { MarketSizingResult, MarketSize, MarketDynamics } from '../../models/competitive';
  ...
  describe('checkForChanges', () => {
    it('should detect no changes for stable market', async () => {
      ...
      changes.forEach(change => {
        ...
        expect(change.type).toMatch(
          /^(growth-rate-shift|market-expansion|competitive-landscape|regulatory-change|technology-disruption|economic-shift)$/
        );
      });
    });
    ...
    it('should identify data gaps', () => {
      ...
      expect(
        result.dataGaps.some(
          gap => gap.includes('Market research') || gap.includes('Industry report')
        )
      ).toBe(true);
    });
  });
  ...
  describe('custom configuration', () => {
    it('should use custom monitoring configuration', () => {
      const customConfig: Partial<MarketMonitoringConfig> = {
        ...
        marketSizeThreshold: 0.1, // 10% instead of default 20%
...
describe('Utility Functions', () => {
  describe('checkMultipleMarketConditions', () => {
    ...
    it('should use custom configuration for multiple market sizings', async () => {
      ...
      const marketSizings = [{ id: 'market-1', result: createMockMarketSizing() }];
...
function createMockMarketSizing(): MarketSizingResult {
  ...
  return {
    ...
    sam: createMockMarketSize('SAM', 2000000000, 0.1),
...
function createMockMarketDynamics(): MarketDynamics {
  return {
    growthDrivers: ['Digital transformation', 'Increasing demand', 'Technology advancement'],
    marketBarriers: ['High competition', 'Regulatory compliance', 'Customer acquisition costs'],
    seasonality: [
      {
        period: 'Q4',
        impact: 1.2,
    ...
    cyclicalFactors: ['Economic cycles', 'Technology adoption cycles'],
    disruptiveForces: ['AI automation', 'New business models', 'Market consolidation'],
  };
}
