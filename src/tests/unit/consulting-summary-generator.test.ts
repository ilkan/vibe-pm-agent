...
describe('ConsultingSummaryGenerator', () => {
  ...
  describe('generateConsultingSummary', () => {
    ...
    it('should generate technique insights for all provided techniques', () => {
      ...
      expect(summary.techniquesApplied.map(t => t.techniqueName)).toEqual([
        'MECE',
        'ValueDriverTree',
        'ZeroBased',
      ]);
  ...
  describe('formatTechniqueInsights', () => {
    ...
    it('should provide default insights for unknown techniques', () => {
      ...
      expect(insight.actionableRecommendation).toBe(
        'Review technique-specific recommendations in detailed analysis'
      );
    });
  });
  ...
  describe('createExecutiveSummary', () => {
    ...
    it('should handle different implementation complexity levels', () => {
      ...
      expect(lowSummary.expectedImpact).toContain(
        'Low implementation complexity ensures rapid deployment'
      );
      expect(highSummary.expectedImpact).toContain(
        'High implementation complexity demands phased rollout'
      );
    });
