...
describe('IntentInterpreter', () => {
  ...
  describe('parseIntent', () => {
    ...
    it('should handle complex intents with multiple features', async () => {
      const rawText =
        'I need a user management system with authentication, user profiles, search functionality, and analytics dashboard';
    ...
    it('should adjust operation costs based on user volume parameters', async () => {
      ...
      const highVolumeTotalCost = highVolumeResult.operationsRequired.reduce(
        (sum, op) => sum + op.estimatedQuotaCost,
        0
      );
      const lowVolumeTotalCost = lowVolumeResult.operationsRequired.reduce(
        (sum, op) => sum + op.estimatedQuotaCost,
        0
      );
    ...
    it('should adjust operation costs based on performance sensitivity', async () => {
      ...
      const highPerfTotalCost = highPerfResult.operationsRequired.reduce(
        (sum, op) => sum + op.estimatedQuotaCost,
        0
      );
      const lowPerfTotalCost = lowPerfResult.operationsRequired.reduce(
        (sum, op) => sum + op.estimatedQuotaCost,
        0
      );
    ...
    it('should adjust technical requirements based on cost constraints', async () => {
      ...
      const hasReducedImpact = result.technicalRequirements.some(
        req => req.quotaImpact === 'minimal' || req.quotaImpact === 'moderate'
      );
    ...
    it('should adjust risk likelihood based on performance sensitivity', async () => {
      ...
      const highPerfAvgLikelihood =
        highPerfResult.potentialRisks.reduce((sum, risk) => sum + risk.likelihood, 0) /
        highPerfResult.potentialRisks.length;
      const lowPerfAvgLikelihood =
        lowPerfResult.potentialRisks.reduce((sum, risk) => sum + risk.likelihood, 0) /
        lowPerfResult.potentialRisks.length;
  ...
  describe('identifyTechnicalRequirements', () => {
    ...
    it('should handle multiple data sources with appropriate complexity', async () => {
      const rawText =
        'Create a dashboard that pulls data from our database, external APIs, and file storage';
      const intent = await interpreter.parseIntent(rawText);
    ...
    it('should merge duplicate requirements and take higher complexity', async () => {
      const rawText =
        'Create a complex user management system with authentication, user profiles, advanced search, and data export';
      const intent = await interpreter.parseIntent(rawText);
