...
describe('Amazon Working Backwards Performance Optimization', () => {
  ...
  describe('Performance Monitor System', () => {
    ...
    it('should detect performance violations', async () => {
      ...
      const { report } = await performanceMonitor.timeOperation(
        operationName,
        async () => {
  ...
  describe('End-to-End Performance', () => {
    ...
    it('should show performance improvements with caching on second run', async () => {
      ...
      const secondLedgerResult = await assumptionLedgerService.normalizeLedger(sampleBusinessInputs);
      ...
      console.log(`Performance improvement: ${Math.round((1 - secondRunDuration / firstRunDuration) * 100)}%`);
    });
  });
  ...
  describe('Performance Benchmarking', () => {
    ...
    it('should provide detailed performance metrics and cache statistics', () => {
      ...
      expect(performanceTargets.some(t => t.operationName === 'assumption_ledger_service')).toBe(true);
